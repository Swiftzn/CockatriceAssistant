...
def parse_cod(path: str) -> CockatriceDeck:
    ...
    banner_card_el = root.find("bannerCard")
    banner_card = banner_card_el.text if banner_card_el is not None else ""

    deck = CockatriceDeck(deckname=deckname, banner_card=banner_card)
...
def write_cod(deck: CockatriceDeck, path: str) -> None:
    ...
    # Add banner card if specified
    if deck.banner_card:
        banner = etree.SubElement(root, "bannerCard", providerId="")
        banner.text = deck.banner_card
